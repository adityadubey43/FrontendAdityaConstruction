      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="w-full max-w-lg rounded-3xl bg-[#1b1b1f] p-6 shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                <div className="text-lg font-semibold">{isEditing ? 'Edit Expense' : 'Add Expense'}</div>
                <div className="text-xs text-white/60">{isEditing ? 'Update the expense record.' : 'Record a new expense.'}</div>
              </div>
              <button onClick={() => {
                setShowModal(false)
                setIsEditing(false)

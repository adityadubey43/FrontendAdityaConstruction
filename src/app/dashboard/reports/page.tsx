...
export default function AdvancedReportsPage() {
  ...
  const { totalOverdue, cashFlowIn, cashFlowOut } = useMemo(() => {
    ...
    bills.forEach((b) => {
      ...
      if (
        b.status === "overdue" ||
        (b.status !== "paid" && new Date(b.dueDate) < now)
      ) {
  ...
  const projectStats = useMemo(() => {
    return projects
      .map((p) => {
        ...
        const margin =
          pPayments > 0 ? ((profit / pPayments) * 100).toFixed(1) : 0;
  ...
  const GenerateInsights = () => {
    ...
    const mostProfitable = [...projectStats]
      .filter((p) => p.profit > 0)
      .sort((a, b) => b.profit - a.profit)[0];
    if (mostProfitable)
      insights.push({
        type: "success",
        text: `"${mostProfitable.projectName}" is driving the highest profit (₹${mostProfitable.profit.toLocaleString()}).`,
      });

    const overBudgetSites = projectStats.filter((p) => p.isOverBudget);
    if (overBudgetSites.length > 0)
      insights.push({
        type: "danger",
        text: `${overBudgetSites.length} project(s) including "${overBudgetSites[0].projectName}" have drastically crossed their estimated budget.`,
      });
    ...
    if (topVendor)
      insights.push({
        type: "warning",
        text: `Vendor "${topVendor.name}" has the highest accumulated spending (₹${topVendor.totalPaid.toLocaleString()}).`,
      });
    ...
    if (totalDelay > 0)
      insights.push({
        type: "danger",
        text: `${totalDelay} active site(s) are officially delayed past their target end date.`,
      });
  ...
  const getTrendData = () => {
    ...
    return Object.values(pData).map((d) => ({
      ...d,
    ...
    <div className="space-y-6">
      ...
      {error && (
        <motion.div
          ...
          className="rounded-2xl bg-red-500/10 border border-red-500/20 p-4 text-red-400 text-sm flex items-center gap-2 font-medium"
        >
      ...
      <section className="relative overflow-hidden group">
        ...
        <div className="glass rounded-[24px] p-6 shadow-2xl border border-white/10 relative z-0">
          <div className="flex items-center gap-2 text-xl font-black mb-6 tracking-tight text-white/90">
            <span className="text-primary text-2xl">⚡</span> Top Auto-Generated
            Insights
          </div>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            <AnimatePresence>
              {insightsList.map((insight, idx) => (
                ...
                >
                  {insight.type === "danger" && (
                    <AlertTriangle className="h-6 w-6 mb-3 opacity-90 drop-shadow-md text-red-400" />
                  )}
                  {insight.type === "warning" && (
                    <AlertTriangle className="h-6 w-6 mb-3 opacity-90 drop-shadow-md text-amber-400" />
                  )}
                  {insight.type === "success" && (
                    <TrendingUp className="h-6 w-6 mb-3 opacity-90 drop-shadow-md text-green-400" />
                  )}
                  <div className="text-[15px] font-semibold tracking-wide leading-relaxed">
                    {insight.text}
                  </div>
                </motion.div>
              ))}
              {insightsList.length === 0 && (
                <div className="text-white/40 text-sm italic col-span-4">
                  No critical insights detected currently. Operating smoothly.
                </div>
              )}
            </AnimatePresence>
          </div>
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
              <div className="bg-[#1a1a1e] rounded-2xl p-5 border border-white/5 shadow-inner">
                <div className="text-xs text-white/40 mb-2 tracking-widest uppercase font-semibold">
                  Expected Inflow
                </div>
                <div className="text-2xl lg:text-2xl font-normal text-green-400">
                  ₹{(cashFlowIn / 1000).toFixed(1)}k
                </div>
              </div>
              <div className="bg-[#1a1a1e] rounded-2xl p-5 border border-white/5 shadow-inner">
                <div className="text-xs text-white/40 mb-2 tracking-widest uppercase font-semibold">
                ...
                <div className="text-2xl lg:text-2xl font-normal text-red-400">
              ...
              <div className="bg-[#1a1a1e] rounded-2xl p-5 border border-white/5 shadow-inner">
                ...
                <div className="text-2xl lg:text-2xl font-normal text-amber-400">
                  ₹{(cashFlowOut / 1000).toFixed(1)}k
                </div>
              </div>
              <div className="bg-[#1a1a1e] rounded-2xl p-5 border border-white/5 shadow-inner">
                <div className="text-xs text-white/40 mb-2 tracking-widest uppercase font-semibold">
                ...
                <div
                  className={`text-2xl lg:text-2xl font-normal ${cashFlowIn - cashFlowOut >= 0 ? "text-green-400" : "text-red-400"}`}
                >
                ...
                {cashFlowIn + cashFlowOut > 0 ? (
                  <>
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(cashFlowIn / (cashFlowIn + cashFlowOut)) * 100}%`,
                      }}
                    ...
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{
                        width: `${(cashFlowOut / (cashFlowIn + cashFlowOut)) * 100}%`,
                      }}
                      transition={{ duration: 1, delay: 0.2 }}
                      className="h-full bg-gradient-to-r from-amber-600 to-amber-400 rounded-full ml-1"
                    />
                  </>
                ) : (
                  <div className="text-xs w-full text-center text-white/30 pt-0.5">
                    Not enough data points found.
                  </div>
                )}
              </div>
          ...
          <div className="flex-1 space-y-4">
            <div className="group flex items-center justify-between p-5 rounded-2xl bg-gradient-to-br from-red-500/10 to-red-500/5 border border-red-500/20 hover:border-red-500/40 transition-colors">
              <div>
                ...
                <div className="text-xs font-medium text-red-200/60 mt-1">
                  Cost exceeds BOQ limit
                </div>
              </div>
              <div className="text-4xl font-black text-red-400 drop-shadow-md">
                {projectStats.filter((p) => p.isOverBudget).length}
              </div>
            </div>
            ...
            <div className="group flex items-center justify-between p-5 rounded-2xl bg-gradient-to-br from-amber-500/10 to-amber-500/5 border border-amber-500/20 hover:border-amber-500/40 transition-colors">
              <div>
                <div className="text-sm font-bold tracking-wide text-amber-200">
                  Delayed Projects
                </div>
                <div className="text-xs font-medium text-amber-200/60 mt-1">
                  Missed completion target
                </div>
              </div>
              <div className="text-4xl font-black text-amber-400 drop-shadow-md">
                {projectStats.filter((p) => p.isDelayed).length}
              </div>
            </div>
            ...
            <div className="group flex items-center justify-between p-5 rounded-2xl bg-gradient-to-br from-blue-500/10 to-blue-500/5 border border-blue-500/20 hover:border-blue-500/40 transition-colors">
              <div>
                <div className="text-sm font-bold tracking-wide text-blue-200">
                  Overdue Invoices
                </div>
                <div className="text-xs font-medium text-blue-200/60 mt-1">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <section className="glass rounded-3xl p-6 h-[460px] flex flex-col hover:border-white/20 transition-all duration-300 overflow-hidden">
          ...
          <div className="text-sm font-medium text-white/50 mb-4 ml-12">
            Categorized spending footprint
          </div>
            ...
            <tbody className="divide-y divide-white/5">
              {projectStats.length === 0 ? (
                <tr>
                  <td
                    colSpan={8}
                    className="py-12 text-center text-white/40 italic font-medium"
                  >
                    No project analytical data available.
                  </td>
                </tr>
              ) : (
                projectStats.map((p, idx) => (
                  ...
                  >
                    <td className="px-5 py-4 text-white/40 font-semibold">
                      {idx + 1}
                    </td>
                    <td className="px-5 py-4 font-bold text-white/90">
                      <span className="flex items-center gap-2">
                        {p.projectName}
                        {p.isDelayed && (
                          <span className="px-2 py-0.5 rounded text-[10px] uppercase font-black bg-amber-500/20 text-amber-300 border border-amber-500/20">
                            Delayed
                          </span>
                        )}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-right font-medium text-green-400/80">
                      ₹{p.totalPayments.toLocaleString("en-IN")}
                    </td>
                    <td className="px-5 py-4 text-right font-medium text-white/60">
                      ₹{(p.budget || 0).toLocaleString("en-IN")}
                    </td>
                    <td className="px-5 py-4 text-right font-medium text-white/90">
                    ...
                    <td className="px-5 py-4 text-right font-medium">
                      {p.isOverBudget ? (
                        <span className="text-red-400 flex items-center justify-end gap-1.5 bg-red-500/10 px-2 py-1 rounded inline-flex">
                          ...
                          {p.overrunAmt.toLocaleString("en-IN")}
                        </span>
                      ) : (
                    ...
                    <td
                      className={`px-5 py-4 text-right font-black ${p.profit >= 0 ? "text-green-400 drop-shadow-[0_0_8px_rgba(74,222,128,0.3)]" : "text-red-400 drop-shadow-[0_0_8px_rgba(248,113,113,0.3)]"}`}
                    >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex justify-center">
                        <div
                          className={`px-3 py-1.5 rounded-lg text-[13px] font-black tracking-wide
                                        ${
                                          Number(p.margin) > 20
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <section className="glass rounded-3xl p-6 hover:border-white/20 transition-all duration-300">
          <h3 className="text-xl font-bold flex items-center gap-3 mb-6 tracking-tight text-white/90">
            <div className="p-2 bg-orange-500/20 rounded-lg">
              <Truck className="text-orange-400 h-5 w-5" />
            </div>
          ...
          <div className="space-y-4">
            {vendors
              ...
              .map((v) => (
                <div
                  key={v._id}
                  className="flex items-center justify-between p-4 rounded-2xl bg-[#1a1a1e] border border-white/5 shadow-inner"
                >
            ...
            {vendors.length === 0 && (
              <div className="text-center py-6 text-white/40 italic font-medium">
        ...
        <section className="glass rounded-3xl p-6 relative overflow-hidden flex flex-col items-center justify-center text-center border-2 border-dashed border-white/10 bg-[#121215]/50 group cursor-default">
          ...
          <h3 className="text-xl font-black text-white/60 tracking-tight">
            Labor Productivity & Material Watch
          </h3>
          <p className="text-white/40 text-sm font-medium mt-4 max-w-sm leading-relaxed">
            Deep tracking algorithms for exact hours logged per unit of work and
            structural material consumption audits are currently calibrating.
          </p>
